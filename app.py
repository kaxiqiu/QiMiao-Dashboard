...
st.markdown("""
    ...
    .phone-box { background-color: #F0F7FF; padding: 15px; border-radius: 10px; border-left: 5px solid #007BFF; }
    </style>
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("➕ 添加新任务")
    new_task_name = st.text_input("任务名", placeholder="如：体能训练")
    new_task_pts = st.number_input("奖励分", min_value=0, value=5)
...
try:
    ...
    # 历史累计奖金：按日期取每日第一条奖金值进行求和
    lifetime_money = df_history.groupby('日期')['当日奖金'].first().sum()
    completed_days = len(df_history['日期'].unique()) + 1
except:
    lifetime_money = 0.0
    completed_days = 1

c1, c2 = st.columns(2)
c1.metric("已坚持记录", f"第 {completed_days} 天")
c2.metric("总累计奖金", f"¥{lifetime_money:.2f}")
...
# 撤销按钮
if st.button("🔙 撤销上一条打卡", disabled=not st.session_state.undo_stack):
...
for i, task in enumerate(st.session_state.custom_tasks):
    if cols[i % 2].button(f"{task['name']} +{task['points']}", key=f"btn_{i}", use_container_width=True):
...
phone_input = st.number_input("今日手机使用时长 (分钟)", min_value=0, value=120, step=5)
# 计算逻辑：120min得20分，每少5min加1分，每多5min减1分
p_diff = 120 - phone_input
p_points = 20 + (p_diff // 5)
st.write(f"📊 结算：{phone_input}分钟 → **自动积分：{p_points}分**")
if st.button("确认记录手机得分", use_container_width=True):
    st.session_state.undo_stack.append({"score": st.session_state.score, "logs": st.session_state.details.copy()})
    st.session_state.score += p_points
    st.session_state.details.append(f"手机结算({phone_input}min): {p_points}分")
    st.rerun()
...
st.divider()
cw, ch = st.columns(2)
weight = cw.text_input("体重 (kg)", placeholder="0.0")
heart = ch.text_input("心率 (bpm)", placeholder="0")

# --- 11. 同步与结算 (Sync to Google) ---
if st.button("🚀 确认结算并同步到云端", type="primary", use_container_width=True):
    date_label = selected_date.strftime("%Y-%m-%d")
    if not st.session_state.details:
        payloads = [{"日期": date_label, "项目明细": "无记录", "项目分值": 0, "当日总分": st.session_state.score, "当日奖金": reward_val, "体重": weight, "心率": heart}]
    else:
        payloads = []
    ...
    try:
        with st.spinner('正在同步数据...'):
            for p in payloads:
                requests.post(SCRIPT_URL, json=p, timeout=10)
        st.balloons()
        st.success("结算成功！")
        # 重置今日
...
try:
    ...
    st.info("读取历史记录中...")    df_raw['日期'] = pd.to_datetime(df_raw['日期']).dt.date
