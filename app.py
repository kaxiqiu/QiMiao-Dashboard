...
st.markdown("""
    ...
    * { font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", sans-serif !important; }
    ...
    .status-badge { font-size: 18px; color: #FF9500; font-weight: 700; }
    ...
    .stButton > button {
        ...
        background-color: white; color: #000; padding: 12px 15px;
    ...
    /* 撤销图标按钮 */
    .undo-icon button {
        padding: 0px 5px !important; font-size: 20px !important;
...
with st.container(border=True): # 使用官方容器替代手动HTML，防止空白框
...
# --- 7. 打卡清单 ---
t_col, u_col = st.columns([5, 1])
t_col.markdown('<p style="font-weight: 600; color: #8E8E93; margin-top: 10px; margin-left: 5px;">🎯 日常打卡清单</p>', unsafe_allow_html=True)
with u_col:
    st.markdown('<div class="undo-icon">', unsafe_allow_html=True)
    if st.button("🔙", disabled=not st.session_state.undo_stack):
...
with col_log:
    with st.popover("📅 补录数据"):
        rec_date = st.date_input("选择日期", datetime.now())
