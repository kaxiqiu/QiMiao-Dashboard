...
st.markdown("""
    ...
    [data-testid="stHorizontalBlock"] {
        ...
        grid-template-columns: repeat(4, 1fr) !important; /* 核心：平分四等份 */
        gap: 5px !important;
        width: 100% !important;
    }
    [data-testid="column"] {
        width: 100% !important;
        min-width: 0px !important; /* 取消最小宽度限制 */
    }
...
with c3:
    if st.button("🧹清除"):
        st.session_state.score = 0; st.session_state.details = []; st.session_stack = []; st.rerun()
with c4:
    with st.popover("⚙️设置"):
...
if st.button("🚀 确认结算并同步到云端", type="primary"):
    ...
    try:
        ...
        st.balloons(); st.success("存档成功！"); st.session_state.score = 0; st.session_state.details = []; st.session_state.undo_stack = []; st.cache_data.clear(); st.rerun()
